...
def get_constants_from_pkg(pkg_file_path):
    ...
    # [('state_type', 'idle, calculation, finishing ')]  <- tuple access first element as state_types[0][0]
    print(state_types)
    ...
    types = re.findall(
        r":\s*(.*?)\s*:=", constants_file_str, flags=re.IGNORECASE
    )
...
def build_dash_table_vhdl(table_content, header=[[]], prefix="-- "):
    ...
    dash_table_vhdl = [
        c if c != "\n" else "\n" + prefix for c in prefix + dash_table_
    ]  # heading+dash_table]
...
def run(src_folder, def_pkg_path):
    ...
    vhdl_src_files = [
        ...
        if path.isfile(src_folder + f) and f.endswith(".vhd")
    ]
    ...
    for vhdl_file in vhdl_src_files:
        ...
        if len(constants_occur) > 0:
            ...
            dash_table_vhdl = (
                "-- +"
                + "\n-- ".join(map(str, build_table_test(constants_list)))
                + "---+"
            )  # debug
            # print(dash_table_vhdl)
